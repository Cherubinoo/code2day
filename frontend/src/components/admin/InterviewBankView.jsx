...
function InterviewFolderMediaManager({ folderId, media, onAdd, onRemove, onUpdate }) {
  ...
  const startEditMedia = (m) => { setEditingId(m.id); setEditTitle(m.title); setEditDescription(m.description || ''); };

  const saveMediaEdit = async (id) => {
    if (!editTitle.trim()) return;
    try {
      const res = await api.patch(`/admin/v2/interview/folders/media/${id}/`, {
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {media.map((m) => (
            <div key={m.id} style={{ position: 'relative', width: 84 }}>
              ...
              {editingId !== m.id && (
                <button onClick={() => startEditMedia(m)} title="Rename / describe" style={{ position: 'absolute', top: -6, left: -6, width: 20, height: 20, borderRadius: '50%', border: 'none', background: 'var(--olive-700)', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}>
                  <Pencil size={10} />
                </button>
              )}
