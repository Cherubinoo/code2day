...
at Google Drive's thumbnail endpoint (https://drive.google.com/thumbnail?id=...)
to instead point at our own caching image proxy
(/api/aptitude/drive-image/<id>/). Anything bulk-uploaded before that proxy
existed is still hitting Drive directly on every page load, which is
noticeably slower than serving from local disk once cached.
...
existed. Not wired into `migrate`; run it manually. Defaults to a dry run.
...
DRIVE_THUMBNAIL_RE = re.compile(r"^https?://drive\.google\.com/thumbnail\?id=([A-Za-z0-9_-]{20,})")
IMAGE_FIELDS = ["question_image", "option_a_image", "option_b_image", "option_c_image", "option_d_image"]


class Command(BaseCommand):
    help = "Rewrite AptitudeQuestion image fields from direct Drive thumbnail URLs to our caching proxy URLs."
    ...
    def handle(self, *args, **options):
        ...
        for field in IMAGE_FIELDS:
            cond = Q(**{f"{field}__startswith": "https://drive.google.com/thumbnail?"})
            filters = filters | cond if filters is not None else cond

        questions = AptitudeQuestion.objects.filter(filters).order_by("id")
        total = questions.count()
        if total == 0:
            self.stdout.write(self.style.WARNING("No questions with direct Drive thumbnail URLs found — nothing to do."))
            return
        ...
        with transaction.atomic():
            for q in questions.iterator():
                ...
                for field in IMAGE_FIELDS:
                    ...
                    m = DRIVE_THUMBNAIL_RE.match(value or "")
                    if m:
